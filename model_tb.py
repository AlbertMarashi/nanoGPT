...
class ThinkingBlock(nn.Module):
    MAX_ITER = 1
    INITIAL_ENERGY_BUDGET = 2.0
    ENERGY_SCALE_FACTOR = 1.0

    def __init__(self, config):
        super().__init__()
        self.block = Block(config)
        self.energy_budget = nn.Parameter(torch.tensor(self.INITIAL_ENERGY_BUDGET))

    def forward(self, x):
        B, T, C = x.shape
        # print("Initial B, T, C:", B, T, C)
        ...
        for i in range(self.MAX_ITER):
            ...
            if i > 0:
                ...
                energy_budget = F.relu(self.energy_budget)
                _, top_indices = torch.topk(energy, k=int(energy_budget), dim=-1)  # select top k based on energy_budget

                # Construct a new mask for updates
                update_mask = torch.zeros_like(entropy, dtype=torch.bool)
                update_mask.scatter_(1, top_indices, True)

                # print(update_mask.shape)

                # Print energy and mask information
                sorted_energy = torch.sort(energy, descending=True).values
                cumulative_energy = torch.cumsum(sorted_energy, dim=-1)
                num_tokens_to_update = torch.sum(cumulative_energy <= energy_budget, dim=-1)
                # print(f"iteration {i}, cumulative_energy: {cumulative_energy.mean():2.2f}, num_tokens_to_update: {num_tokens_to_update.sum():,}")


            # Apply the block(s) to the tokens that require further processing
            # Expand mask to cover channel dimension
            update_mask = update_mask.unsqueeze(-1).expand(-1, -1, C)

            # Select values to update
            x_to_update = x.where(update_mask, torch.zeros_like(x))

            # Process updates
            x_updated = self.block(x_to_update)

            # Scatter updated values back into x only where the mask is True
            x = torch.where(update_mask, x_updated, x)
